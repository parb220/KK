...
!    whose correlation is R, that X .gt. AH and Y .gt. AK.
!
!  Modified:
!
!    23 November 2010
...
!  Parameters:
!
!    Input, real ( kind = 8 ) AH, AK, the limits of integration.
!
!    Input, real ( kind = 8 ) R, the correlation.
!
...
!  gauss is a univariate lower normal
!  tail area calculated here from the
!  central error function derf.
!
!  It may be replaced by the algorithm in
!  hill,i.d. and joyce,s.a. algorithm 304,
!  normal curve integral(515), comm.a.c.m.(10)
!  (june,1967),p.374.
!  source: owern, d.b. ann.math.stat.
!  vol. 27(1956), p.1075.
!
  b = 0.0D+00
...
!
!  AH = AK = 0.0.
!  The original code had a mistake here, JVB, 23 November 2010.
!
...
210   continue
      ...
      if ( wk .eq. 0.0D+00 ) then
        go to 320
      end if

      if ( abs ( wk ) - 1.0D+00 ) 270, 230, 240
