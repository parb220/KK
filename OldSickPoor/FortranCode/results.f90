    ...
    subroutine ComputeExPostStats1()
        ...
        do ti=1,nw
            ...
            do i = 1,na  
                em = mod(eh+1,2)+1        
                ef = int((eh+1)/2)              
                MedicareRevenue = MedicareRevenue +  (MedicareTax(w*efmat(jf,jm,ef,ti)*labWcube(i,jf,jm,vf,vm,eh,ti)) + MedicareTax(w*emmat(jf,jm,em,ti)*hbar)) * psiW(i,jf,jm,vf,vm,eh,ti)*CohortWeights(ti)        
            end do
            end do
            end do 
            end do
       ...
       call Out(0,"TotalPayments", TotalPayments, ret)
        ...
        do ti=1,nr
            FracIndByMaritalStatusAge(1,ti) = MaritalDist(1,ti)*2.0d0/(MaritalDist(1,ti)*2.0d0 + MaritalDist(2,ti) + MaritalDist(3,ti)) 
            FracIndByMaritalStatusAge(2,ti) = MaritalDist(2,ti)/(MaritalDist(1,ti)*2.0d0 + MaritalDist(2,ti) + MaritalDist(3,ti))
            FracIndByMaritalStatusAge(3,ti) = MaritalDist(3,ti)/(MaritalDist(1,ti)*2.0d0 + MaritalDist(2,ti) + MaritalDist(3,ti)) 
        end do
        ...
        do i=1,na
            if (labWcube(i,ef,em,vf,vm,het,ti) > 0.0d0) then
                FracWomenLabForByAgeEduc(ti,het) = FracWomenLabForByAgeEduc(ti,het) + psiW(i,ef,em,vf,vm,het,ti)/educDist(het) 
                AveFemaleHoursCondWorkByEduc(het) = AveFemaleHoursCondWorkByEduc(het) + labWcube(i,ef,em,vf,vm,het,ti)*psiW(i,ef,em,vf,vm,het,ti)*CohortWeights(ti)/educDist(het)                                                               
            end if
            AveFemaleHoursByAgeEduc(ti,het) = AveFemaleHoursByAgeEduc(ti,het) + labWcube(i,ef,em,vf,vm,het,ti)*psiW(i,ef,em,vf,vm,het,ti)/educDist(het)           
        end do
        end do
        end do
        end do
        ...
        do i=1,na
            ...
            if (labWcube(i,ef,em,vf,vm,het,ti) > 0.0d0) then  
                ...
                tempR = (1-labWcube(i,ef,em,vf,vm,het,ti))/labWcube(i,ef,em,vf,vm,het,ti)/gamma*psiW(i,ef,em,vf,vm,het,ti)/FracWomenLabForByAge(ti)*CohortWeights(ti) 
                !if (.NOT. ieee_is_nan(tempR)) then
                !if (.NOT. isnan(tempR)) then
                if (tempR .ne. tempR) then
            ...
            AveFemaleEarnings = AveFemaleEarnings + w*labWcube(i,ef,em,vf,vm,het,ti)*efmat(ef,em,fet,ti)*psiW(i,ef,em,vf,vm,het,ti)*CohortWeights(ti) 
            AveHHEarnings = AveHHEarnings + w*labWcube(i,ef,em,vf,vm,het,ti)*efmat(ef,em,fet,ti)*psiW(i,ef,em,vf,vm,het,ti)*CohortWeights(ti) &
             +  w*hbar*emmat(ef,em,met,ti)*psiW(i,ef,em,vf,vm,het,ti)*CohortWeights(ti) 
            AveMaleEarningsCheck = AveMaleEarningsCheck + w*hbar*emmat(ef,em,met,ti)*psiW(i,ef,em,vf,vm,het,ti)*CohortWeights(ti) 
        end do
        end do
        end do
        end do
        ...
        AggConsAtDeath = sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeMarried(:,:,:,:,:,4,:) * psiRMarried(:,:,:,:,:,4,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidow(:,:,:,:,:,2,:) * psiRWidow(:,:,:,:,:,2,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidower(:,:,:,:,:,2,:) * psiRWidower(:,:,:,:,:,2,:),5),4),3),2),1),1)
        
        
        AggConsExcludeDeath = AggCons - AggConsAtDeath  
        
        AggConsNHResidents = sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeMarried(:,5,:,:,:,:,:) * psiRMarried(:,5,:,:,:,:,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidow(:,5,:,:,:,:,:) * psiRWidow(:,5,:,:,:,:,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidower(:,5,:,:,:,:,:) * psiRWidower(:,5,:,:,:,:,:),5),4),3),2),1),1)
        AggConsNHResidents = AggConsNHResidents + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeMarried(:,10,:,:,:,:,:) * psiRMarried(:,10,:,:,:,:,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidow(:,10,:,:,:,:,:) * psiRWidow(:,10,:,:,:,:,:),5),4),3),2),1),1) & 
                  + sum(CohortWeights(nw+1:)*sum(sum(sum(sum(sum(consumptionRcubeWidower(:,10,:,:,:,:,:) * psiRWidower(:,10,:,:,:,:,:),5),4),3),2),1),1)
        ...
        do s = 1,3
         ...
         do i = 1,na 
             if (s==1) then
                ...
                ConsByAgeID(nw+ti) = ConsByAgeID(nw+ti)  +  sum(consumptionRCubeMarried(i,j,vf,vm,h,:,ti) * psiRMarried(i,j,vf,vm,h,:,ti)) 
            else if (s==2) then
                WealthByAgeID2(nw+ti) = WealthByAgeID2(nw+ti) + avect(i) * sum(psiRWidow(i,j,vf,vm,h,:,ti))   
                ConsByAgeID(nw+ti) = ConsByAgeID(nw+ti)  + sum(consumptionRCubeWidow(i,j,vf,vm,h,:,ti) * psiRWidow(i,j,vf,vm,h,:,ti))     
            else
                WealthByAgeID2(nw+ti) = WealthByAgeID2(nw+ti) + avect(i) * sum(psiRWidower(i,j,vf,vm,h,:,ti)) 
                ConsByAgeID(nw+ti) =  ConsByAgeID(nw+ti)  + sum(consumptionRCubeWidower(i,j,vf,vm,h,:,ti) * psiRWidower(i,j,vf,vm,h,:,ti))       
            end if                         
         end do
         end do
         end do 
        ...
        do ef=1,nef
        do i=1,na
            EarnsFbyAgeID(ti) = EarnsFbyAgeID(ti) + w*efmat(ef,em,fet,ti)*labWcube(i,ef,em,vf,vm,het,ti)*psiW(i,ef,em,vf,vm,het,ti) 
            ...
            preTaxIncomeM = w*emmat(ef,em,met,ti)*hbar - SocSecTax(w*emmat(ef,em,met,ti)*hbar) + 0.5d0*r* avect(i) &
                - EarnsTaxFn(w*emmat(ef,em,met,ti)*hbar)                                     
        ...
        do ti=1,nr 
            ...
            do i=1,na                        
                ...
                if(TransferRet == 0.0d0) then
                    ...
                    if (j==5 .or. j==10) then
                        ConsNHPrivate(ti) = ConsNHPrivate(ti) + consumptionRCubeMarried(i,j,vf,vm,h,d,1)*psiRMarried(i,j,vf,vm,h,d,ti)
                ...
                !    else
                         OOPExpArrayMarried(i,j,vf,vm,h,d,ti) = min((1-FracMedicaidCovers)*mmat(j,h,d,s,ti) ,clowerbarMarried/2)                
                !    end if
                    if (j==5 .or. j==10) then
                        ConsNHMedicaid(ti) = ConsNHMedicaid(ti) + consumptionRCubeMarried(i,j,vf,vm,h,d,1)*psiRMarried(i,j,vf,vm,h,d,ti)
            ...
            do i=1,na             
                ...
                if(TransferRet == 0.0d0) then
                    ...
                    if (j==5 .or. j==10) then
                        ConsNHPrivate(ti) = ConsNHPrivate(ti) + consumptionRCubeWidow(i,j,vf,vm,h,d,1)*psiRWidow(i,j,vf,vm,h,d,ti)
                ...
                !    else
                        OOPExpArrayWidow(i,j,vf,vm,h,d,ti) = min((1-FracMedicaidCovers)*mmat(j,h,d,s,ti) ,clowerbarWidow/2)      
                !    end if   
                    if (j==5 .or. j==10) then
                        ConsNHMedicaid(ti) = ConsNHMedicaid(ti) + consumptionRCubeWidow(i,j,vf,vm,h,d,1)*psiRWidow(i,j,vf,vm,h,d,ti)
            ...
            do i=1,na             
                ...
                if(TransferRet == 0.0d0) then
                    ...
                    if (j==5 .or. j==10) then
                        ConsNHPrivate(ti) = ConsNHPrivate(ti) + consumptionRCubeWidower(i,j,vf,vm,h,d,1)*psiRWidower(i,j,vf,vm,h,d,ti)
                ...
                !    else
                        OOPExpArrayWidower(i,j,vf,vm,h,d,ti) = min((1-FracMedicaidCovers)*mmat(j,h,d,s,ti) ,clowerbarWidower/2)      
                !    end if
                    if (j==5 .or. j==10) then
                        ConsNHMedicaid(ti) = ConsNHMedicaid(ti) + consumptionRCubeWidower(i,j,vf,vm,h,d,1)*psiRWidower(i,j,vf,vm,h,d,ti)
                        MedExpsNHMedicaid(ti) = MedExpsNHMedicaid(ti) + mmat(j,h,d,s,ti)*psiRWidower(i,j,vf,vm,h,d,ti) 
        ...
        !ByAge stats
        do ti = 1,nr     
            FracHHTopMedShockByAge(ti) = FracHHTopMedShockByAge(ti) + sum(psiRMarried(:,5,:,:,:,:,ti)) + sum(psiRMarried(:,10,:,:,:,:,ti)) + &
         ...
         do i = 1,na
            ...
            if (transfersRetired(postTaxWealth, ti, j,d,h,s,i) >0.0d0) then
                if (s==1) then
                    ...
                    MeanOOPExpsMedicaidByAge(ti) =  MeanOOPExpsMedicaidByAge(ti) + OOPExpenses*psiRMarried(i,j,vf,vm,h,d,ti)    
                else if (s==2) then
                    ...
                    MeanOOPExpsMedicaidByAge(ti) = MeanOOPExpsMedicaidByAge(ti) +OOPExpenses*psiRWidow(i,j,vf,vm,h,d,ti)  
                else
                    ...
                    MeanOOPExpsMedicaidByAge(ti) = MeanOOPExpsMedicaidByAge(ti) + OOPExpenses*psiRWidower(i,j,vf,vm,h,d,ti)   
                end if                
            end if              
            if (s==1) then
        ...
        FracIndMedicaidByAge = FracWidowsMedicaidByAge*FracIndByMaritalStatusAge(2,:) + FracWidowersMedicaidByAge*FracIndByMaritalStatusAge(3,:) + &
        ...
        Frac6566WidowsMedicaid = sum(FracWidowsMedicaidByAge(1:2)*CohortWeights(nw+1:nw+2))/sum(CohortWeights(nw+1:nw+2))
        Frac6574WidowsMedicaid = sum(FracWidowsMedicaidByAge(1:5)*CohortWeights(nw+1:nw+5))/sum(CohortWeights(nw+1:nw+5))    
        Frac7584WidowsMedicaid = sum(FracWidowsMedicaidByAge(6:10)*CohortWeights(nw+6:nw+10))/sum(CohortWeights(nw+6:nw+10))    
        Frac85plusWidowsMedicaid = sum(FracWidowsMedicaidByAge(11:)*CohortWeights(nw+11:))/sum(CohortWeights(nw+11:))    
        
        FracWidowersMedicaid = sum(FracWidowersMedicaidByAge*CohortWeights(nw+1:))/sum(CohortWeights(nw+1:))  
        Frac6566WidowersMedicaid = sum(FracWidowersMedicaidByAge(1:2)*CohortWeights(nw+1:nw+2))/sum(CohortWeights(nw+1:nw+2))  
        Frac6574WidowersMedicaid = sum(FracWidowersMedicaidByAge(1:5)*CohortWeights(nw+1:nw+5))/sum(CohortWeights(nw+1:nw+5))
        Frac7584WidowersMedicaid = sum(FracWidowersMedicaidByAge(6:10)*CohortWeights(nw+6:nw+10))/sum(CohortWeights(nw+6:nw+10))    
        Frac85plusWidowersMedicaid = sum(FracWidowersMedicaidByAge(11:)*CohortWeights(nw+11:))/sum(CohortWeights(nw+11:))    
        
        FracMarriedMedicaid = sum(FracMarriedMedicaidByAge*CohortWeights(nw+1:))/sum(CohortWeights(nw+1:))
        Frac6566MarriedMedicaid = sum(FracMarriedMedicaidByAge(1:2)*CohortWeights(nw+1:nw+2))/sum(CohortWeights(nw+1:nw+2))
        Frac6574MarriedMedicaid = sum(FracMarriedMedicaidByAge(1:5)*CohortWeights(nw+1:nw+5))/sum(CohortWeights(nw+1:nw+5))
        Frac7584MarriedMedicaid = sum(FracMarriedMedicaidByAge(6:10)*CohortWeights(nw+6:nw+10))/sum(CohortWeights(nw+6:nw+10))    
        Frac85plusMarriedMedicaid = sum(FracMarriedMedicaidByAge(11:)*CohortWeights(nw+11:))/sum(CohortWeights(nw+11:))    
        ...
        Frac6574HHTopMedShock = sum(FracHHTopMedShockByAge(1:5)*CohortWeights(nw+1:nw+5))/sum(CohortWeights(nw+1:nw+5))
        Frac7584HHTopMedShock = sum(FracHHTopMedShockByAge(6:10)*CohortWeights(nw+6:nw+10))/sum(CohortWeights(nw+6:nw+10))
        Frac85plusHHTopMedShock = sum(FracHHTopMedShockByAge(11:)*CohortWeights(nw+11:))/sum(CohortWeights(nw+11:))        
        ...
        MedicaidExpOverOutputByAge(1) = sum(CohortWeights(nw+1:nw+5) * (TotMedByAgeID(:5) - MeanOOPExpByAge(:5)),1)/AggOutput*100
        MedicaidExpOverOutputByAge(2) = sum(CohortWeights(nw+6:nw+10) * (TotMedByAgeID(6:10) - MeanOOPExpByAge(6:10)),1)/AggOutput*100
        MedicaidExpOverOutputByAge(3) = sum(CohortWeights(nw+11:) * (TotMedByAgeID(11:) - MeanOOPExpByAge(11:)),1)/AggOutput*100                        
        ...
        do ti=1,nr
            do d=1,2
            do h = 1,nsht
                AveNHExpensesWidows(ti) = AveNHExpensesWidows(ti) + mmat(5,h,d,2,ti)*sum(psiRWidow(:,5,:,:,h,d,ti)) &
                    + mmat(10,h,d,2,ti)*sum(psiRWidow(:,10,:,:,h,d,ti))   
                do j=1,4
                AveNonNHExpensesWidows(ti) = AveNonNHExpensesWidows(ti) + mmat(j,h,d,2,ti)*sum(psiRWidow(:,j,:,:,h,d,ti)) &
                    + mmat(5+j,h,d,2,ti)*sum(psiRWidow(:,5+j,:,:,h,d,ti)) 
                end do
                do j=1,10
            ...
            AggNHExp = AggNHExp + CohortWeights(nw+ti) * (AveNHExpensesWidows(ti)+AveNHExpensesWidowers(ti)+AveNHExpensesMarried(ti))
                 
            AveNHExpensesWidows(ti) = AveNHExpensesWidows(ti)/(sum(psiRWidow(:,5,:,:,1:2,1:2,ti))+sum(psiRWidow(:,10,:,:,1:2,1:2,ti)))
            AveNonNHExpensesWidows(ti) = AveNonNHExpensesWidows(ti)/(sum(psiRWidow(:,1:4,:,:,1:2,1:2,ti))+ sum(psiRWidow(:,6:10,:,:,1:2,1:2,ti)))
            AveExpensesWidows(ti) = AveExpensesWidows(ti)/sum(psiRWidow(:,:,:,:,1:2,1:2,ti)) 
            
            AveNHExpensesWidowers(ti) = AveNHExpensesWidowers(ti)/(sum(psiRWidower(:,5,:,:,1:2,1:2,ti))+sum(psiRWidower(:,10,:,:,1:2,1:2,ti)))
            AveNonNHExpensesWidowers(ti) = AveNonNHExpensesWidowers(ti)/(sum(psiRWidower(:,1:4,:,:,1:2,1:2,ti))+ sum(psiRWidower(:,6:10,:,:,1:2,1:2,ti)))
            AveExpensesWidowers(ti) = AveExpensesWidowers(ti)/sum(psiRWidower(:,:,:,:,1:2,1:2,ti))  
            
            AveNHExpensesMarried(ti) = AveNHExpensesMarried(ti)/(sum(psiRMarried(:,5,:,:,:,:,ti))+sum(psiRMarried(:,10,:,:,:,:,ti)))
            AveNonNHExpensesMarried(ti) = AveNonNHExpensesMarried(ti)/(sum(psiRMarried(:,1:4,:,:,:,:,ti))+ sum(psiRMarried(:,6:10,:,:,:,:,ti)))
        ...
        do ti=1,nr
            ...
            AveTotExpsByMart(1) = AveTotExpsByMart(1) + sum(mmat(:,:,:,1,ti)*sum(sum(sum(psiRMarried(:,:,:,:,:,:,ti),4),3),1))/MaritalDist(1,ti)*CohortWeights(nw+ti)        
            AveOOPExpsByMart(1) = AveOOPExpsByMart(1) + sum(OOPExpArrayMarried(:,:,:,:,:,:,ti)*psiRMarried(:,:,:,:,:,:,ti))/MaritalDist(1,ti)*CohortWeights(nw+ti)              
            do h=1,nhht   
                AveTotExpsByHealMart(h,1) = AveTotExpsByHealMart(h,1) + sum(mmat(:,h,:,1,ti)*sum(sum(sum(psiRMarried(:,:,:,:,h,:,ti),4),3),1)/sum(sum(sum(psiRMarried(:,:,:,:,h,:,ti),4),3),1))*CohortWeights(nw+ti)   
                AveOOPExpsByHealMart(h,1) = AveOOPExpsByHealMart(h,1) + sum(OOPExpArrayMarried(:,:,:,:,h,:,ti)*psiRMarried(:,:,:,:,h,:,ti)/psiRMarried(:,:,:,:,h,:,ti))*CohortWeights(nw+ti)          
            end do    
            AveTotExpsByMart(2) = AveTotExpsByMart(2) + sum(mmat(:,1:2,1:2,2,ti)*sum(sum(sum(psiRWidow(:,:,:,:,:,:,ti),4),3),1))/MaritalDist(2,ti)*CohortWeights(nw+ti)        
            AveOOPExpsByMart(2) = AveOOPExpsByMart(2) + sum(OOPExpArrayWidow(:,:,:,:,:,:,ti)*psiRWidow(:,:,:,:,:,:,ti))/MaritalDist(2,ti)*CohortWeights(nw+ti)              
            do h=1,nsht   
                AveTotExpsByHealMart(h,2) = AveTotExpsByHealMart(h,2) + sum(mmat(:,h,1:2,2,ti)*sum(sum(sum(psiRWidow(:,:,:,:,h,:,ti),4),3),1)/sum(sum(sum(psiRWidow(:,:,:,:,h,:,ti),4),3),1))*CohortWeights(nw+ti)   
                AveOOPExpsByHealMart(h,2) = AveOOPExpsByHealMart(h,2) + sum(OOPExpArrayWidow(:,:,:,:,h,:,ti)*psiRWidow(:,:,:,:,h,:,ti)/psiRWidow(:,:,:,:,h,:,ti))*CohortWeights(nw+ti)          
            end do  
            AveTotExpsByMart(3) = AveTotExpsByMart(3) + sum(mmat(:,1:2,1:2,3,ti)*sum(sum(sum(psiRWidower(:,:,:,:,:,:,ti),4),3),1))/MaritalDist(3,ti)*CohortWeights(nw+ti)        
            AveOOPExpsByMart(3) = AveOOPExpsByMart(3) + sum(OOPExpArrayWidower(:,:,:,:,1:2,:,ti)*psiRWidower(:,:,:,:,:,:,ti))/MaritalDist(3,ti)*CohortWeights(nw+ti)              
            do h=1,nsht   
                AveTotExpsByHealMart(h,3) = AveTotExpsByHealMart(h,3) + sum(mmat(:,h,1:2,3,ti)*sum(sum(sum(psiRWidower(:,:,:,:,h,:,ti),4),3),1)/sum(sum(sum(psiRWidower(:,:,:,:,h,:,ti),4),3),1))*CohortWeights(nw+ti)   
                AveOOPExpsByHealMart(h,3) = AveOOPExpsByHealMart(h,3) + sum(OOPExpArrayWidower(:,:,:,:,h,:,ti)*psiRWidower(:,:,:,:,h,:,ti)/psiRWidower(:,:,:,:,h,:,ti))*CohortWeights(nw+ti)          
            end do                                       
