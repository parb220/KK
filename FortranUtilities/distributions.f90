    module distributions
     ...
     subroutine GetInitialDistribution(grid1, grid2, stdev1, stdev2, corr, dist)
        implicit none
        real(dbl), dimension(:), intent(in):: grid1
        real(dbl), dimension(:), intent(in):: grid2
        real(dbl), intent(in):: stdev1, stdev2, corr
        ...
        do i=2,n1-1
        do j=2,n2-1
            dist(i,j) = bivnor((grid1(i)-w1)/stdev1,(grid2(j)-w2)/stdev2,corr) &
                        - bivnor((grid1(i)+w1)/stdev1,(grid2(j)-w2)/stdev2,corr) &
                        - bivnor((grid1(i)-w1)/stdev1,(grid2(j)+w2)/stdev2,corr) &
                        + bivnor((grid1(i)+w1)/stdev1,(grid2(j)+w2)/stdev2,corr)
        ...
        do i=1,n1
            dist(i,1) = (1-enordf( (grid1(i) - w1)/stdev1 )) - (1-enordf( (grid1(i) + w1)/stdev1 )) &
                         - bivnor((grid1(i)-w1)/stdev1,(grid2(1)+w2)/stdev2,corr) + &
                         bivnor((grid1(i)+w1)/stdev1,(grid2(1)+w2)/stdev2,corr)
                         
            dist(i,n2) = bivnor((grid1(i)-w1)/stdev1,(grid2(n2)-w2)/stdev2,corr) - &
                         bivnor((grid1(i)+w1)/stdev1,(grid2(n2)-w2)/stdev2,corr)                    
        end do
        do j=1,n2
            dist(1,j) = (1-enordf( (grid2(j) - w2)/stdev2 )) - (1-enordf( (grid2(j) + w2)/stdev2 )) &
                         - bivnor((grid1(1)+w1)/stdev1,(grid2(j)-w2)/stdev2,corr) &
                         + bivnor((grid1(1)+w1)/stdev1,(grid2(j)+w2)/stdev2,corr)
            dist(n1,j) = bivnor((grid1(n1)-w1)/stdev1,(grid2(j)-w2)/stdev2,corr) &
            - bivnor((grid1(n1)-w1)/stdev1,(grid2(j)+w2)/stdev2,corr)    
        end do
        dist(1,1) = 1 - (1-enordf( (grid1(1) + w1)/stdev1 )) - (1-enordf( (grid2(1) + w2)/stdev2 )) &
                    + bivnor((grid1(1)+w1)/stdev1,(grid2(1)+w2)/stdev2,corr)
        ...
        dist(1,n2) = (1-enordf( (grid2(n2) - w2)/stdev2 )) &
        - bivnor((grid1(1)+w1)/stdev1,(grid2(n2)-w2)/stdev2,corr)
        dist(n1,1) = (1-enordf( (grid1(n1) - w1)/stdev1 )) &
        - bivnor((grid1(n1)-w1)/stdev1,(grid2(1)+w2)/stdev2,corr)
        dist(n1,n2) = bivnor((grid1(n1)-w1)/stdev1,(grid2(n2)-w2)/stdev2,corr)
