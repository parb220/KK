...
!***END PROLOGUE  DNSQE
      INTEGER INDEX, INFO, IOPT, J, LR, LWA, MAXFEV, ML, MODE, MU, N,NFEV, NJEV, NPRINT
      DOUBLE PRECISION EPSFCN, FACTOR, FVEC(*), ONE, TOL, WA(*),X(*), XTOL, ZERO
      EXTERNAL FCN, JAC
...
!   set of DATA statements should be activated by removing the ! from
...
!     ...EXIT
         ...
         IF (FNORM < 1.e-8) INFO = 1
!     .........EXIT
         IF (INFO .NE. 0) PRINT *, "FNORM < 1.e-8 EXITING..."
...
!           ...EXIT
               ...
               IF (MODE .EQ. 2) GO TO 70
                  DO 60 J = 1, N
                     DIAG(J) = WA2(J)
! Replace the zero comparison with an inequality
!                     IF (WA2(J) .EQ. ZERO) DIAG(J) = ONE
                     IF (DABS(WA2(J)) .LT. 1D-10) DIAG(J) = ONE
   60             CONTINUE
   70          CONTINUE
!
...
!               IF (DELTA .EQ. ZERO) DELTA = FACTOR
               IF (DABS(DELTA) .LT.FACTOR) DELTA = FACTOR
   90       CONTINUE
!
...
!               IF (FJAC(J,J) .EQ. ZERO) GO TO 130
               IF (DABS(FJAC(J,J)) .lt. 1d-16) GO TO 130
                  SUM = ZERO
                  DO 110 I = J, N
...
!               IF (WA1(J) .EQ. ZERO) SING = .TRUE.
               IF (DABS(WA1(J)) .LT. 1D-16) SING = .TRUE.
  170       CONTINUE
!
...
!               IF (DELTA .LE. XTOL*XNORM .OR. FNORM .EQ. ZERO) INFO = 1
               IF (DELTA .LE. XTOL*XNORM .OR. DABS(FNORM) .LT. 1D-16) &
	                INFO = 1
!     .........EXIT
               IF (INFO .NE. 0) GO TO 320
!
...
!         IF (WA(K) .EQ. ZERO) GO TO 110
         IF (DABS(WA(K)) .LT. 1D-16) GO TO 110
         DO 100 J = K, M
            SUM = ZERO
            DO 80 I = K, M
               SUM = SUM + Q(I,J)*WA(I)
...
!         IF (AJNORM .EQ. ZERO) GO TO 100
         IF (DABS(AJNORM) .LT. 1D-16) GO TO 100
...
!            IF (.NOT.PIVOT .OR. SIGMA(K) .EQ. ZERO) GO TO 80
            IF (.NOT.PIVOT .OR. DABS(SIGMA(K)) .LT. 1D-16) GO TO 80
            TEMP = A(J,K)/SIGMA(K)
...
!         IF (W(J) .EQ. ZERO) GO TO 120
         IF (DABS(W(J)) .LT. 1D-16) GO TO 120
!
!        DETERMINE A GIVENS ROTATION WHICH ELIMINATES THE
...
!         IF (S(JJ) .EQ. ZERO) SING = .TRUE.
         IF (DABS(S(JJ)) .LT. 1D-16) SING = .TRUE.
         JJ = JJ + (M - J + 1)
  130    CONTINUE
  140 CONTINUE
!
...
!         IF (TEMP .EQ. ZERO) TEMP = EPSMCH
         IF (DABS(TEMP) .LT. EPSMCH) TEMP = EPSMCH
   40    CONTINUE
         X(J) = (QTB(J) - SUM)/TEMP
   50    CONTINUE
!
...
!      IF (GNORM .EQ. ZERO) GO TO 120
      IF (DABS(GNORM) .LT. 1D-16) GO TO 120
!
...
!      IF (S1 .EQ. ZERO) GO TO 100
      IF (DABS(S1) .LT. 1D-16) GO TO 100
         DENORM = X1MAX*SQRT(S1+(S2/X1MAX)/X1MAX)
         GO TO 130
  100 CONTINUE
! Replace the zero comparison with an inequality
!         IF (S2 .EQ. ZERO) GO TO 110
         IF (DABS(S2) .LT. 1D-16) GO TO 110
            IF (S2 .GE. X3MAX) &
